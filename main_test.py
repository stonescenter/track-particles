...
def main():
    ...
    print('[Data] shape: X%s y%s :' % (X.shape, y.shape))
    # reshape data     
    ...
    conv_pred = data.convert_supervised_to_normal(y_pred.values, n_hit_in=4, n_hit_out=1, hits=10)
    conv_pred = pd.DataFrame(conv_pred)
    ...
    if cylindrical:
        ...
        conv_true.to_csv(os.path.join(output_path, 'y_true_%s_xyz.csv' % configs['model']['name']),
                    header=False, index=False)
        conv_pred.to_csv(os.path.join(output_path, 'y_pred_%s_xyz.csv' % configs['model']['name']),
                    header=False, index=False)
